...
interface Livreur {
  ...
  create_package_config?: Record<string, unknown> | null;
}
interface Hub {
  id: number;
  name: string;
}
interface HubLivreur {
  hub_id: number;
  livreur_id: string;
}
...
const KeyValueEditor = ({
  ...
  useEffect(() => {
    setPairs(Object.entries(safeRecord(value)));
  }, [value]);
    ...
    onChange(
      JSON.stringify(
        Object.fromEntries(
          nextPairs
            .filter(([key]) => key.trim())
            .map(([key, item]) => [key.trim(), primitiveValues ? toPrimitive(item) : item]),
        ),
        null,
        2,
      ),
        ...
        {(pairs.length ? pairs : [["", ""]]).map(([key, item], index) => (
          <div key={`${key}-${index}`} className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_40px]">
            <Input
              ...
              onChange={(e) =>
                updatePairs(
                  (pairs.length ? pairs : [["", ""] as [string, string]]).map((pair, i): [string, string] =>
...
const COMMON_WEBHOOK_PATHS: string[] = [
  ...
  "scheduledDate",
  "deliveryDate",
  "transport.currentDriverName",
  "transport.currentDriverPhone",
  "driver.name",
  "driver.phone",
  "driver_name",
  "driver_phone",
...
const SmartMappingEditor = ({
  ...
  const emit = (nextPairs: Array<[string, string]>) =>
    onChange(
      JSON.stringify(Object.fromEntries(nextPairs.filter(([k]) => k.trim()).map(([k, v]) => [k.trim(), v])), null, 2),
    );
  const updatePairs = (nextPairs: Array<[string, string]>) => {
    setPairs(nextPairs);
    emit(nextPairs);
    ...
    <div className="space-y-3">
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <Input
            value={String(auth.type ?? "none")}
            onChange={(e) => update({ type: e.target.value })}
            placeholder="none or token"
          />
          <FieldHelp>Use none if the provider does not require authentication.</FieldHelp>
        </div>
        <div>
          <Label>Method</Label>
          <Input
            value={String(auth.method ?? "POST")}
            onChange={(e) => update({ method: e.target.value })}
            placeholder="POST"
          />
          <FieldHelp>HTTP method used for the authentication request.</FieldHelp>
        </div>
      </div>
      <div>
        <Label>Auth URL</Label>
        <Input
          value={String(auth.url ?? "")}
          onChange={(e) => update({ url: e.target.value })}
          placeholder="https://..."
        />
        <FieldHelp>Login or token endpoint from the provider.</FieldHelp>
      </div>
      <KeyValueEditor
        label="Auth headers"
        help="Headers sent only with the authentication request."
        value={formatJson(auth.headers ?? {})}
        onChange={(headers) => update({ headers: safeRecord(headers) })}
        keyPlaceholder="Header"
        valuePlaceholder="Value"
      />
      <KeyValueEditor
        label="Auth payload mapping"
        help="Left side is the provider auth field. Right side can be an order field or secret:SECRET_NAME."
        value={formatJson(auth.payload_mapping ?? {})}
        onChange={(payload_mapping) => update({ payload_mapping: safeRecord(payload_mapping) })}
        keyPlaceholder="Provider field"
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <Input
            value={String(auth.response_token_path ?? "token")}
            onChange={(e) => update({ response_token_path: e.target.value })}
          />
          <FieldHelp>Where the token is found in the auth response.</FieldHelp>
        </div>
        <div>
          <Label>Token header</Label>
          <Input
            value={String(auth.token_header ?? "Authorization")}
            onChange={(e) => update({ token_header: e.target.value })}
          />
          <FieldHelp>Header used later for protected requests.</FieldHelp>
        </div>
        <div>
          <Label>Token prefix</Label>
          <Input
            value={String(auth.token_prefix ?? "Bearer ")}
            onChange={(e) => update({ token_prefix: e.target.value })}
          />
          <FieldHelp>Common value is Bearer with a trailing space.</FieldHelp>
        </div>
        <div>
          <Label>Expires in path</Label>
          <Input
            value={String(auth.expires_in_path ?? "expiresIn")}
            onChange={(e) => update({ expires_in_path: e.target.value })}
          />
          <FieldHelp>Optional response field for token lifetime.</FieldHelp>
        </div>
      </div>
    </div>
  );
...
const ApiOperationsEditor = ({ value, onChange }: { value: string; onChange: (value: string) => void }) => {
  ...
  const updateOperation = (index: number, patch: Record<string, unknown>) =>
    onChange(
      JSON.stringify(
        operations.map((operation, i) => (i === index ? { ...operation, ...patch } : operation)),
        null,
        2,
      ),
    );
  ...
  return (
    <div className="space-y-3">
      <div>
        <Label>Extra API operations</Label>
        <FieldHelp>
          Requêtes additionnelles déclenchées avant/après création, au changement de statut, planifiées ou récurrentes.
        </FieldHelp>
      </div>
          ...
          <div key={index} className="space-y-3 rounded-md border border-border p-3">
              ...
              <div className="flex items-center gap-2">
                <label className="flex items-center gap-1 text-xs text-muted-foreground">
                  <Switch
                    ...
                    onCheckedChange={(v) => updateOperation(index, { enabled: v })}
                  />{" "}
                  Activé
                </label>
            ...
            <div className="grid gap-3 sm:grid-cols-[1fr_120px]">
              <div>
                ...
                <Input
                  value={String(operation.url ?? "")}
                  onChange={(e) => updateOperation(index, { url: e.target.value })}
                  placeholder="https://..."
                />
              </div>
              <div>
                ...
                <Select
                  value={String(operation.method ?? "POST")}
                  onValueChange={(v) => updateOperation(index, { method: v })}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div className="rounded-md border border-dashed border-border p-3 space-y-3 bg-muted/30">
              ...
              {trigger === "on_status_change" && (
                <div>
                  ...
                  <Select
                    value={String(operation.trigger_status ?? "Pickup")}
                    onValueChange={(v) => updateOperation(index, { trigger_status: v })}
                  >
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
              ...
              {trigger === "scheduled" && (
                <div>
                  ...
                  <Input
                    type="datetime-local"
                    value={String(operation.scheduled_at ?? "")}
                    onChange={(e) => updateOperation(index, { scheduled_at: e.target.value })}
                  />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      type="number"
                      min={1}
                      value={Number(operation.interval_value ?? 60)}
                      onChange={(e) => updateOperation(index, { interval_value: Number(e.target.value) })}
                    />
                  </div>
                  <div>
                    <Label className="text-xs">Unité</Label>
                    <Select
                      value={String(operation.interval_unit ?? "minutes")}
                      onValueChange={(v) => updateOperation(index, { interval_unit: v })}
                    >
                      <SelectTrigger>
                        <SelectValue />
                      </SelectTrigger>
              ...
              {(trigger === "scheduled" || trigger === "recurring") && (
                <p className="text-[11px] text-emerald-600 dark:text-emerald-400">
                  ✓ Planificateur actif (exécution toutes les minutes).
                </p>
              )}
            </div>
            ...
            <KeyValueEditor
              label="Headers"
              help="Headers spécifiques à cette opération."
              value={formatJson(operation.headers ?? {})}
              onChange={(headers) => updateOperation(index, { headers: safeRecord(headers) })}
            />
            <KeyValueEditor
              label="Payload mapping"
              help="Mapping des champs envoyés. Utilisez {{create_response.field}} pour récupérer un champ de la création."
              value={formatJson(operation.payload_mapping ?? {})}
              onChange={(payload_mapping) => updateOperation(index, { payload_mapping: safeRecord(payload_mapping) })}
              keyPlaceholder="Champ provider"
              valuePlaceholder="Champ commande ou {{template}}"
            />

            {/* NOUVEAU CHAMP: Payload JSON (optionnel) */}
            <div className="mt-3">
              <Label className="text-xs">Payload JSON (optionnel)</Label>
              <Textarea
                className="font-mono text-xs min-h-20"
                placeholder='{ "packages": ["{{create_response.trackingID}}"], "driverEmail": "" }'
                value={
                  typeof operation.payload_json === "string"
                    ? operation.payload_json
                    : operation.payload_json
                      ? JSON.stringify(operation.payload_json, null, 2)
                      : ""
                }
                onChange={(e) => updateOperation(index, { payload_json: e.target.value })}
              />
              <FieldHelp>
                Si renseigné, ce JSON remplace le mapping clé/valeur ci-dessus. Utilisez{" "}
                {"{{create_response.trackingID}}"} pour récupérer le numéro de suivi.
              </FieldHelp>
            </div>

            {operation.payload && (
              <div>
                <Label className="text-xs">Payload JSON brut (optionnel - legacy)</Label>
                <Textarea
                  className="font-mono text-xs min-h-20"
                  value={
                  ...
                  onChange={(e) => {
                    try {
                      updateOperation(index, { payload: JSON.parse(e.target.value) });
                    } catch {
                      updateOperation(index, { payload: e.target.value });
                    }
                  }}
                />
              </div>
      ...
      <Button type="button" variant="outline" size="sm" onClick={addOperation}>
        <Plus className="mr-1 h-4 w-4" /> Ajouter une opération
      </Button>
    </div>
  );
};
...
const AdminLivreurs = () => {
  ...
  const filteredLogs = useMemo(
    () =>
      apiLogs.filter((log) => {
        if (
          logFilter !== "all" &&
          (logFilter === "webhook" ? log.event_type !== "webhook_status" : log.event_type === "webhook_status")
        )
          return false;
  ...
  const detectedProviderFields = useMemo(() => {
    ...
    const configuredWebhookPaths = [
      settingsForm?.webhook_tracking_field,
      settingsForm?.webhook_status_field,
      settingsForm?.webhook_note_field,
      settingsForm?.webhook_reported_date_field,
      settingsForm?.webhook_scheduled_date_field,
      settingsForm?.webhook_driver_name_field,
      settingsForm?.webhook_driver_phone_field,
    ].filter(Boolean) as string[];
    const configuredPollingPaths = [
      settingsForm?.polling_tracking_field,
      settingsForm?.polling_status_field,
      settingsForm?.polling_message_field,
      settingsForm?.polling_reported_date_field,
  ...
  const load = async () => {
    const [p, h, hl, s, logs, retentionSetting] = await Promise.all([
      db
        .from("profiles")
        .select("id, username, full_name, api_enabled, api_token, authentication_config, create_package_config")
        .eq("role", "livreur")
        .order("username"),
      ...
      db
        .from("livreur_api_logs")
        .select("id, order_id, livreur_id, event_type, status, message, details, created_at")
    ...
    (s.data ?? []).forEach((row: LivreurApiSettings) => {
      byLivreur[row.livreur_id] = row;
    });
    setSettings(byLivreur);
    setApiLogs(logs.data ?? []);
    const retentionValue = (retentionSetting.data?.value ?? {}) as Record<string, unknown>;
    setRetention({
      enabled: Boolean(retentionValue.enabled),
      hours: Number(retentionValue.hours ?? (Number(retentionValue.days) || 3) * 24) || 72,
    });
  };
  useEffect(() => {
    load();
  ...
  const toggleApi = async (l: Livreur, v: boolean) => {
    ...
    if (error) toast.error(error.message);
      ...
      toast.info(
        v
          ? "API enabled: confirmed orders will be sent to the driver's provider to create external tracking."
          : "API disabled: the app will generate internal tracking numbers instead of creating packages through the provider.",
      );
      load();
    }
  };
  ...
  const toggleWebhook = async (l: Livreur, v: boolean) => {
    ...
    const { error } = await db
      ...
      .upsert({ ...current, livreur_id: l.id, webhook_enabled: v }, { onConflict: "livreur_id" });
    if (error) toast.error(error.message);
    else {
      toast.info(
        v
          ? "Webhook enabled: incoming provider notifications will be accepted, logged, and used according to this driver's API/polling setup."
          : "Webhook disabled: incoming provider notifications will be logged as ignored and will not update orders.",
      );
      load();
    }
  };
  ...
  const saveSettings = async () => {
    ...
    try {
      const payload = {
        ...
        webhook_order_fields_mapping: parseJson(
          "Webhook order fields",
          (settingsForm as any).webhook_order_fields_mapping,
        ),
        ...
        polling_driver_name_field:
          ((settingsForm as any).polling_driver_name_field || "").trim() || "transport.currentDriverName",
        polling_driver_phone_field:
          ((settingsForm as any).polling_driver_phone_field || "").trim() || "transport.currentDriverPhone",
        polling_extra_fields_mapping: parseJson(
          "Polling extra fields",
          (settingsForm as any).polling_extra_fields_mapping,
        ),
        polling_order_fields_mapping: parseJson(
          "Polling order fields",
          (settingsForm as any).polling_order_fields_mapping,
        ),
      ...
      const { error: profileError } = await db
        .from("profiles")
        .update({ authentication_config: authConfig, create_package_config: createPackageConfig })
        .eq("id", editing.id);
  ...
  const saveRetention = async () => {
    ...
    const { error } = await db
      .from("app_settings")
      .upsert({ key: "api_logs_retention", value: { enabled: retention.enabled, hours } }, { onConflict: "key" });
    if (error) toast.error(error.message);
    else {
      toast.success("Log cleanup settings saved");
      setRetention({ enabled: retention.enabled, hours });
    }
  };
  ...
  const getLogTracking = (log: (typeof apiLogs)[number]) => {
    ...
    const candidates = [
      details.tracking,
      details.expected_tracking,
      details.response_tracking,
      details.reception?.payload?.trackingID,
      details.reception?.payload?.tracking,
      details.reception?.payload?.partnerTrackingID,
      details.reception?.body?.trackingID,
      details.reception?.body?.tracking,
      details.reception?.body?.partnerTrackingID,
    ];
    ...
    <>
                  ...
                  <TableRow key={l.id}>
                    ...
                    <TableCell>
                      <div className="flex flex-col gap-2">
                        ...
                        <label className="flex items-center gap-2 text-sm">
                          <Switch
                            checked={(settings[l.id] as any)?.webhook_enabled ?? false}
                            onCheckedChange={(v) => toggleWebhook(l, v)}
                          />
                          <span>Webhook</span>
                        </label>
                      </div>
                    ...
                    <TableCell>
                      <div className="flex items-center gap-2">
                        <Input
                          ...
                          value={show.has(l.id) ? l.api_token || "—" : masked(l.api_token)}
                        />
                        <Button
                          variant="ghost"
                          size="icon"
        ...
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Showing latest {apiLogs.length} receptions, rejections, polling checks, and provider responses with full
              details. Cleanup retention is measured in hours.
            </p>
          </div>
          <div className="flex flex-wrap items-center justify-end gap-2">
            <Input
              className="h-9 w-64"
              placeholder="Search order, tracking, status..."
              value={logSearch}
            ...
            <Select value={logFilter} onValueChange={setLogFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="webhook">Webhook logs</SelectItem>
                <SelectItem value="driver">Driver API logs</SelectItem>
              </SelectContent>
            </Select>
            <label className="flex items-center gap-2 rounded-md border border-border px-3 py-2 text-sm">
              <Switch
                checked={retention.enabled}
                onCheckedChange={(enabled) => setRetention({ ...retention, enabled })}
              />{" "}
              Auto clean
            </label>
            <div className="flex items-center gap-2">
              <Input
                ...
                onChange={(e) => setRetention({ ...retention, hours: Number(e.target.value) })}
              />
              ...
              filteredLogs.map((log) => {
                ...
                return (
                  <TableRow key={log.id}>
                    ...
                    <TableCell className="font-mono text-xs">{getLogTracking(log)}</TableCell>
                    <TableCell>{livreur?.full_name || livreur?.username || "—"}</TableCell>
                    <TableCell>
                      {log.event_type}
                      <div className="mt-1 flex flex-wrap gap-1">
                        <Badge variant="outline">Reception {flow.reception ? "✓" : "—"}</Badge>
                        <Badge variant="outline">Sending {flow.sending ? "✓" : "—"}</Badge>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge
      ...
      <Dialog open={!!selectedLog} onOpenChange={(v) => !v && setSelectedLog(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Log full details</DialogTitle>
          </DialogHeader>
          {selectedLog && (
            <div className="space-y-3 text-sm">
                  ...
                  <div className="grid gap-3 lg:grid-cols-2">
                    <div>
                      ...
                      <pre className="max-h-[30vh] overflow-auto rounded-md bg-muted p-3 text-xs">
                        {JSON.stringify(flow.reception ?? "No reception data saved for this legacy log", null, 2)}
                      </pre>
                    </div>
                    <div>
                      <Label>Sending</Label>
                      <pre className="max-h-[30vh] overflow-auto rounded-md bg-muted p-3 text-xs">
                        {JSON.stringify(flow.sending ?? "No sending data saved for this legacy log", null, 2)}
                      </pre>
                    </div>
                    {flow.exchanges.length > 0 && (
                      <div className="lg:col-span-2">
                        <Label>All request/response exchanges</Label>
                        <pre className="max-h-[32vh] overflow-auto rounded-md bg-muted p-3 text-xs">
                          {JSON.stringify(flow.exchanges, null, 2)}
                        </pre>
                      </div>
                    )}
                  </div>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <div>
                  <Label>Time</Label>
                  <p className="rounded-md bg-muted p-2">{new Date(selectedLog.created_at).toLocaleString("fr-FR")}</p>
                </div>
                <div>
                  <Label>Event</Label>
                  <p className="rounded-md bg-muted p-2">{selectedLog.event_type}</p>
                </div>
                <div>
                  <Label>Order</Label>
                  <p className="rounded-md bg-muted p-2">{selectedLog.order_id ?? "—"}</p>
                </div>
                <div>
                  <Label>Status</Label>
                  <p className="rounded-md bg-muted p-2">{selectedLog.status}</p>
                </div>
              </div>
              <div>
                <Label>Message</Label>
                <p className="rounded-md bg-muted p-2">{selectedLog.message ?? "—"}</p>
              </div>
              <div>
                <Label>Endpoint details</Label>
                <pre className="max-h-[32vh] overflow-auto rounded-md bg-muted p-3 text-xs">
                  {JSON.stringify(getLogEndpointInfo(selectedLog), null, 2)}
                </pre>
              </div>
              <div>
                <Label>Details</Label>
                <pre className="max-h-[45vh] overflow-auto rounded-md bg-muted p-3 text-xs">
          ...
          <DialogFooter>
            {selectedLog && (
              <Button type="button" variant="destructive" onClick={() => deleteLog(selectedLog.id)}>
      ...
      <Dialog open={!!editing} onOpenChange={(v) => !v && setEditing(null)}>
        <DialogContent className="max-w-6xl max-h-[92vh] overflow-y-auto">
          <DialogHeader>
            ...
            <div className="mt-3 rounded-md border border-border bg-muted/40 p-4 text-sm text-muted-foreground">
              <div className="flex gap-3">
                ...
                <div className="space-y-2">
                  <p>
                    API and Webhook are independent. API creates external packages. Webhook receives provider
                    notifications. Polling fetches provider status on schedule.
                  </p>
                  <p>
                    If API and Webhook are both enabled, webhook notifications are logged first, then polling remains
                    responsible for fetching the full current order status.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid gap-4 xl:grid-cols-2">
            <Card className="p-4 space-y-4">
              <SectionHeader
                ...
                description="Main request used to create a delivery package with the selected provider."
              />
              <div className="grid gap-3 sm:grid-cols-[1fr_140px]">
                <div>
                  <Label>Create package URL</Label>
                  <Input
                    value={settingsForm.create_package_url}
                    onChange={(e) => setSettingsForm({ ...settingsForm, create_package_url: e.target.value })}
                    placeholder="https://..."
                  />
                  <FieldHelp>Endpoint provided by the delivery company to create a new package.</FieldHelp>
                </div>
                <div>
                  <Label>Method</Label>
                  <Input
                    value={settingsForm.create_package_method}
                    onChange={(e) => setSettingsForm({ ...settingsForm, create_package_method: e.target.value })}
                  />
                  <FieldHelp>Usually POST.</FieldHelp>
                </div>
              </div>
              <div>
                <Label>Response tracking path</Label>
                <Input
                  value={settingsForm.response_tracking_path}
                  onChange={(e) => setSettingsForm({ ...settingsForm, response_tracking_path: e.target.value })}
                  placeholder="trackingID"
                />
                <FieldHelp>
                  Where the tracking number is found in the create package response. Use nested paths like
                  data.trackingID when needed.
                </FieldHelp>
              </div>
              <KeyValueEditor
                label="Headers"
                help="Optional headers sent with the package creation request. Add one key/value per row."
                value={settingsForm.create_package_headers}
                onChange={(value) => setSettingsForm({ ...settingsForm, create_package_headers: value })}
                keyPlaceholder="Content-Type"
                valuePlaceholder="application/json"
              />
              <SmartMappingEditor
                label="Payload mapping"
                help="Left = provider field name (free text or detected from past responses). Right = order field sent as the value. Use Add row to capture every body field freely."
                value={settingsForm.create_package_mapping}
                onChange={(value) => setSettingsForm({ ...settingsForm, create_package_mapping: value })}
                keyOptions={detectedProviderFields.createPackage}
                valueOptions={SYSTEM_ORDER_FIELDS}
                keyPlaceholder="Provider field"
                valuePlaceholder="Order field"
              />
            </Card>
            <Card className="p-4 space-y-4">
              <SectionHeader
                ...
                description="Optional login/token request used before calling protected provider endpoints."
              />
              <AuthConfigEditor
                value={settingsForm.auth_config}
                onChange={(value) => setSettingsForm({ ...settingsForm, auth_config: value })}
              />
              <ApiOperationsEditor
                value={settingsForm.api_operations}
                onChange={(value) => setSettingsForm({ ...settingsForm, api_operations: value })}
              />
              <div>
                ...
                <Input
                  ...
                  min={0.1}
                  step={0.1}
                  value={settingsForm.rate_limit_per_second}
                  onChange={(e) => setSettingsForm({ ...settingsForm, rate_limit_per_second: Number(e.target.value) })}
                />
                <FieldHelp>
                  Maximum outgoing requests per second for this provider. Set it according to the provider limit.
                </FieldHelp>
              </div>
            </Card>
            <Card className="p-4 space-y-4">
              <SectionHeader
                ...
                description="Rules checked before sending, plus status mapping for incoming webhook updates."
              />
              <KeyValueEditor
                label="Validation rules"
                help="Input rules such as minimum product length, phone digits, or minimum order value. Values can be plain text, numbers, true/false, or small JSON objects."
                value={settingsForm.validation_rules}
                onChange={(value) => setSettingsForm({ ...settingsForm, validation_rules: value })}
                keyPlaceholder="Order field"
                valuePlaceholder='Rule, e.g. {"min_alnum":3}'
                primitiveValues
              />
              <KeyValueEditor
                label="Status mapping (Webhook)"
                help="Used only for incoming webhook notifications. Left = provider status as sent in the webhook body, right = internal status used in this app."
                value={settingsForm.status_mapping}
                onChange={(value) => setSettingsForm({ ...settingsForm, status_mapping: value })}
                keyPlaceholder="Provider status"
                valuePlaceholder="Internal status"
              />
              <div>
                <Label>Webhook URL</Label>
                <Input readOnly value={editing ? `${functionsBaseUrl}/livreur-webhook/${editing.id}` : ""} />
                <FieldHelp>Give this URL to the provider with this driver's API token as a Bearer token.</FieldHelp>
              </div>
              <label className="flex items-center justify-between gap-3 rounded-md border border-border p-3 text-sm">
                <span>Enable webhook reception</span>
                <Switch
                  checked={settingsForm.webhook_enabled}
                  onCheckedChange={(v) => setSettingsForm({ ...settingsForm, webhook_enabled: v })}
                />
              </label>
              <div className="grid gap-3 sm:grid-cols-2">
                <SmartFieldPath
                  label="Webhook status field"
                  help="Field name that contains the provider status in the webhook body."
                  value={settingsForm.webhook_status_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, webhook_status_field: v })}
                  options={detectedProviderFields.webhook}
                />
                <SmartFieldPath
                  label="Webhook tracking field"
                  help="Field name that contains the tracking number in the webhook body."
                  value={settingsForm.webhook_tracking_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, webhook_tracking_field: v })}
                  options={detectedProviderFields.webhook}
                />
                <SmartFieldPath
                  label="Webhook driver name field"
                  help="Path used to capture the driver name shown in order details."
                  value={settingsForm.webhook_driver_name_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, webhook_driver_name_field: v })}
                  options={detectedProviderFields.webhook}
                />
                <SmartFieldPath
                  label="Webhook driver phone field"
                  help="Path used to capture the driver phone shown in order details."
                  value={settingsForm.webhook_driver_phone_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, webhook_driver_phone_field: v })}
                  options={detectedProviderFields.webhook}
                />
                <SmartFieldPath
                  label="Webhook note field"
                  help="Path used to capture the delivery note."
                  value={settingsForm.webhook_note_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, webhook_note_field: v })}
                  options={detectedProviderFields.webhook}
                />
                <SmartFieldPath
                  label="Webhook date Reporté field"
                  help="Path used to capture postponed delivery date."
                  value={settingsForm.webhook_reported_date_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, webhook_reported_date_field: v })}
                  options={detectedProviderFields.webhook}
                />
                <SmartFieldPath
                  label="Webhook date Programmé field"
                  help="Path used to capture scheduled delivery date."
                  value={settingsForm.webhook_scheduled_date_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, webhook_scheduled_date_field: v })}
                  options={detectedProviderFields.webhook}
                />
                <SmartFieldPath
                  label="Webhook activity actor field"
                  help="Path used to capture the actor displayed in the activity timeline. Use 'lastmsg' to take the last entry's message from a history array, or 'history.last.user' for the user of the last entry, or any custom path."
                  value={(settingsForm as any).webhook_actor_field || "lastmsg"}
                  onChange={(v) => setSettingsForm({ ...settingsForm, webhook_actor_field: v } as any)}
                  options={["lastmsg", "history.last.msg", "history.last.user", ...detectedProviderFields.webhook]}
                />
              </div>
              <SmartMappingEditor
                label="Webhook extra fields"
                help="Capture extra info from incoming webhook bodies. Left = a key you choose (free text). Right = the webhook path, picked from fields detected in this driver's recent webhook receptions."
                value={settingsForm.webhook_extra_fields_mapping}
                onChange={(value) => setSettingsForm({ ...settingsForm, webhook_extra_fields_mapping: value })}
                keyOptions={[]}
                valueOptions={detectedProviderFields.webhook}
                keyPlaceholder="Saved key"
                valuePlaceholder="Webhook body path"
              />
              <SmartMappingEditor
                label="Webhook → Order columns mapping"
                help="Capture any value from the webhook body and write it directly to an order column. Left = order field (driver_name, comment, status_note, postponed_date, scheduled_date, etc.). Right = webhook body path."
                value={(settingsForm as any).webhook_order_fields_mapping}
                onChange={(value) => setSettingsForm({ ...settingsForm, webhook_order_fields_mapping: value } as any)}
                keyOptions={SYSTEM_ORDER_FIELDS}
                valueOptions={detectedProviderFields.webhook}
                keyPlaceholder="Order field"
                valuePlaceholder="Webhook body path"
              />
              <label className="flex items-center justify-between gap-3 rounded-md border border-border p-3 text-sm">
                <span>Webhook updates current status</span>
                <Switch
                  checked={settingsForm.webhook_updates_current_status}
                  onCheckedChange={(v) => setSettingsForm({ ...settingsForm, webhook_updates_current_status: v })}
                />
              </label>
              <FieldHelp>
                When API and webhook are both enabled, webhook notifications are saved in logs first and polling should
                fetch the complete current status. When only webhook is enabled, this switch lets the webhook body
                update the current order status directly.
              </FieldHelp>
              <label className="flex items-center justify-between gap-3 rounded-md border border-border p-3 text-sm">
                <span>Settings enabled</span>
                <Switch
                  checked={settingsForm.is_active}
                  onCheckedChange={(v) => setSettingsForm({ ...settingsForm, is_active: v })}
                />
              </label>
            </Card>
            <Card className="p-4 space-y-4">
              <SectionHeader
                ...
                description="Use polling when the provider has no webhook. The app checks order status on a schedule."
              />
              <label className="flex items-center justify-between gap-3 rounded-md border border-border p-3 text-sm">
                <span>Enable polling</span>
                <Switch
                  checked={settingsForm.polling_enabled}
                  onCheckedChange={(v) => setSettingsForm({ ...settingsForm, polling_enabled: v })}
                />
              </label>
              <div className="grid gap-3 sm:grid-cols-2">
                <div>
                  ...
                  />
                  <FieldHelp>How often the app checks for status updates.</FieldHelp>
                </div>
                <div>
                  <Label>Method</Label>
                  <Input
                    value={settingsForm.polling_status_method}
                    onChange={(e) => setSettingsForm({ ...settingsForm, polling_status_method: e.target.value })}
                  />
                  <FieldHelp>GET or POST, based on the provider documentation.</FieldHelp>
                </div>
              </div>
              <div>
                <Label>Status URL</Label>
                <Input
                  value={settingsForm.polling_status_url}
                  onChange={(e) => setSettingsForm({ ...settingsForm, polling_status_url: e.target.value })}
                  placeholder="https://..."
                />
                <FieldHelp>Endpoint used to fetch the latest provider status for an order.</FieldHelp>
              </div>
              <KeyValueEditor
                label="Polling headers"
                help="Optional headers sent with the status polling request."
                value={settingsForm.polling_status_headers}
                onChange={(value) => setSettingsForm({ ...settingsForm, polling_status_headers: value })}
                keyPlaceholder="Header"
                valuePlaceholder="Value"
              />
              {String(settingsForm.polling_status_method || "GET").toUpperCase() !== "GET" ? (
                <SmartMappingEditor
                  label="Polling request body mapping"
                  help="Used only when polling method is POST/PUT/PATCH. Left = body key sent to provider. Right = order field used as the value. For GET requests this is ignored — provider data is captured from the response using the field paths below."
                  value={settingsForm.polling_status_payload_mapping}
                  onChange={(value) => setSettingsForm({ ...settingsForm, polling_status_payload_mapping: value })}
                  keyOptions={detectedProviderFields.polling}
                  valueOptions={SYSTEM_ORDER_FIELDS}
                ...
                <div className="rounded-md border border-dashed border-border p-3 text-xs text-muted-foreground">
                  Polling method is GET — no request body is sent. Provider data (status, driver, dates) is captured
                  from the response using the field paths below.
                </div>
              )}
              <div className="grid gap-3 sm:grid-cols-3">
                <SmartFieldPath
                  label="Tracking field"
                  help="Path to tracking number in the polling response."
                  value={settingsForm.polling_tracking_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, polling_tracking_field: v })}
                  options={detectedProviderFields.polling}
                />
                <SmartFieldPath
                  label="Status field"
                  help="Path to provider status in the polling response."
                  value={settingsForm.polling_status_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, polling_status_field: v })}
                  options={detectedProviderFields.polling}
                />
                <SmartFieldPath
                  label="Message field"
                  help="Path to delivery note/message in the polling response."
                  value={settingsForm.polling_message_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, polling_message_field: v })}
                  options={detectedProviderFields.polling}
                />
                <SmartFieldPath
                  label="Date Reporté field"
                  help="Path to postponed delivery date in the polling response."
                  value={settingsForm.polling_reported_date_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, polling_reported_date_field: v })}
                  options={detectedProviderFields.polling}
                />
                <SmartFieldPath
                  label="Date Programmé field"
                  help="Path to scheduled delivery date in the polling response."
                  value={settingsForm.polling_scheduled_date_field}
                  onChange={(v) => setSettingsForm({ ...settingsForm, polling_scheduled_date_field: v })}
                  options={detectedProviderFields.polling}
                />
                <SmartFieldPath
                  label="Driver name field"
                  help="Path to driver name in the polling response."
                  value={(settingsForm as any).polling_driver_name_field || ""}
                  onChange={(v) => setSettingsForm({ ...settingsForm, polling_driver_name_field: v } as any)}
                  options={detectedProviderFields.polling}
                />
                <SmartFieldPath
                  label="Driver phone field"
                  help="Path to driver phone in the polling response."
                  value={(settingsForm as any).polling_driver_phone_field || ""}
                  onChange={(v) => setSettingsForm({ ...settingsForm, polling_driver_phone_field: v } as any)}
                  options={detectedProviderFields.polling}
                />
                <SmartFieldPath
                  label="Activity actor field"
                  help="Path used to fill the actor in the activity timeline. Use 'lastmsg' to take the last entry's message from a history array (e.g. Olivraison), 'history.last.user' for the user of the last entry, or any custom path."
                  value={(settingsForm as any).polling_actor_field || "lastmsg"}
                  onChange={(v) => setSettingsForm({ ...settingsForm, polling_actor_field: v } as any)}
                  options={["lastmsg", "history.last.msg", "history.last.user", ...detectedProviderFields.polling]}
                />
              </div>
              <SmartMappingEditor
                label="Polling extra fields"
                help="Capture extra info from polling responses for logs and debugging. Left = a key you choose. Right = the response path."
                value={(settingsForm as any).polling_extra_fields_mapping}
                onChange={(value) => setSettingsForm({ ...settingsForm, polling_extra_fields_mapping: value } as any)}
                keyOptions={[]}
                valueOptions={detectedProviderFields.polling}
                keyPlaceholder="Saved key"
                valuePlaceholder="Response body path"
              />
              <SmartMappingEditor
                label="Polling → Order columns mapping"
                help="Capture any value from the polling response and write it directly to an order column. Left = order field. Right = response body path."
                value={(settingsForm as any).polling_order_fields_mapping}
                onChange={(value) => setSettingsForm({ ...settingsForm, polling_order_fields_mapping: value } as any)}
                keyOptions={SYSTEM_ORDER_FIELDS}
                valueOptions={detectedProviderFields.polling}
                keyPlaceholder="Order field"
                valuePlaceholder="Response body path"
              />
              <KeyValueEditor
                label="Status mapping (Polling)"
                help="Used only for the scheduled polling job. Left = provider status returned in the polling response, right = internal status used in this app."
                value={settingsForm.polling_status_mapping}
                onChange={(value) => setSettingsForm({ ...settingsForm, polling_status_mapping: value })}
                keyPlaceholder="Provider status"
                valuePlaceholder="Internal status"
              />
            </Card>
          ...
          <DialogFooter>
            ...
            <Button type="button" onClick={saveSettings} disabled={savingId === editing?.id}>
              {savingId === editing?.id ? "..." : "Save"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
